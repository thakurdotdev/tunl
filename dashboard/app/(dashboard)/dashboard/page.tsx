    ...
    <div className="flex flex-col gap-8 font-mono">
      ...
      <div className="border-border/60 flex flex-col gap-4 border-b pb-5 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center gap-3 text-xs">
          <div className="border-border/60 bg-card/80 flex items-center gap-2.5 rounded-lg border px-3.5 py-1.5 shadow-2xs backdrop-blur-xs">
      ...
      <div className="grid gap-6 lg:grid-cols-12">
        ...
        <div className="border-border/60 bg-card/90 hover:border-border/80 flex flex-col overflow-hidden rounded-xl border shadow-xs backdrop-blur-xs transition-all lg:col-span-6">
          <div className="border-border/60 bg-muted/30 flex items-center justify-between border-b px-5 py-3 text-xs">
            ...
            <span className="text-muted-foreground text-[10px] font-semibold uppercase">ALIAS</span>
          </div>

          <div className="flex flex-1 flex-col justify-between gap-5 p-6">
            <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4">
              <div className="flex flex-col gap-2">
                <Label htmlFor="subdomain" className="text-muted-foreground text-xs font-semibold">
                  TARGET SUBDOMAIN ALIAS
                </Label>
                <div className="relative flex items-center">
                  <span className="text-primary absolute left-3.5 text-xs font-bold select-none">
                    $
                  </span>
                  <Input
                    id="subdomain"
                    ...
                    className="bg-background/50 border-border/60 focus-visible:ring-primary/40 h-10 rounded-lg pr-[105px] pl-8 font-mono text-xs"
                    {...register("subdomain")}
                    aria-invalid={!!errors.subdomain}
                  />
                  <span className="text-muted-foreground/70 pointer-events-none absolute right-3.5 font-mono text-xs select-none">
                    .thakur.dev
                  </span>
                </div>
                {errors.subdomain && (
                  <p className="text-destructive font-mono text-xs">{errors.subdomain.message}</p>
                )}
              </div>
              <Button
                type="submit"
                disabled={createMutation.isPending || limitReached}
                size="sm"
                className="h-9 shrink-0 gap-1.5 self-start px-4 font-mono text-xs"
              >
        ...
        <div className="border-border/60 bg-card/90 hover:border-border/80 flex flex-col overflow-hidden rounded-xl border shadow-xs backdrop-blur-xs transition-all lg:col-span-6">
          <div className="border-border/60 bg-muted/30 flex items-center justify-between border-b px-5 py-3 text-xs">
            ...
            <span className="text-primary text-[10px] font-bold">OPENSSH</span>
          </div>

          <div className="flex flex-1 flex-col justify-between gap-4 p-6">
            <div className="flex items-center justify-between gap-4">
              <div className="flex flex-col gap-1">
                <span className="text-foreground text-xs font-semibold">GENERATED SSH COMMAND</span>
              ...
              <div className="flex w-24 shrink-0 flex-col gap-1">
                ...
                <Input
                  ...
                  placeholder="3000"
                  className="bg-background/50 border-border/60 h-7 [appearance:textfield] rounded px-2 text-center font-mono text-xs [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none"
                />
              </div>
            </div>

            <div className="bg-muted/60 text-primary border-border/60 flex items-center gap-3 rounded-lg border p-3 font-mono text-xs select-all">
      ...
      <div className="border-border/60 bg-card/90 flex flex-col overflow-hidden rounded-xl border shadow-xs">
        ...
        {isLoading ? (
          <div className="flex flex-col items-center justify-center gap-2 py-16">
            ...
            <p className="text-muted-foreground font-mono text-xs">Loading subdomains...</p>
          </div>
        ) : tunnels.length === 0 ? (
            ...
            <table className="w-full border-collapse text-left font-mono text-xs">
              <thead>
                <tr className="border-border/60 bg-muted/20 text-muted-foreground border-b text-[11px] tracking-wider uppercase">
                  <th className="w-[220px] p-4 font-semibold">Subdomain Endpoint</th>
                  <th className="w-[120px] p-4 font-semibold">State</th>
                  <th className="p-4 font-semibold">SSH Command Snippet</th>
                  <th className="w-[140px] p-4 font-semibold">Created Date</th>
                  <th className="w-[80px] p-4 text-right font-semibold">Action</th>
                </tr>
              </thead>
              <tbody>
                {tunnels.map((tunnel) => {
                    ...
                    >
                      ...
                      <td className="p-4">
                        <div className="flex max-w-[360px] items-center gap-2">
                          <code className="bg-muted/60 text-primary border-border/60 block flex-1 truncate rounded-md border px-2.5 py-1 font-mono text-[11px] select-all">
                            {sshCommand}
                          </code>
                          <Button
                            variant="outline"
        ...
        {activeSessions.length === 0 ? (
          <div className="flex flex-col items-center justify-center gap-3 px-4 py-14 text-center">
            ...
            <p className="text-muted-foreground font-mono text-xs">
              // No active tunnel sessions currently established
            </p>
          </div>
        ) : (
          <div className="no-scrollbar overflow-x-auto">
              ...
              <tbody>
                {activeSessions.map((session) => (
                  ...
                  >
                    ...
                    <td className="p-4">
                      <span className="bg-primary/10 text-primary border-primary/20 rounded-full border px-2 py-0.5 text-[10px] font-semibold">
        ...
        <AlertDialogContent className="font-mono">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-xs">
              ...
              subdomain will be closed immediately.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
